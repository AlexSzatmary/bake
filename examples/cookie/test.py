...
import os
try:
    p = os.popen('../../bake/cmdline.py -m -l -f bp/many_sprinkles.bp')
    hout = open('batch/test1.txt', 'w')
    hout.write(p.read())
    hout.close()
    p.close()
    p = os.popen('../../bake/cmdline.py -l -f bp/many_sprinkles.bp -s 1-3')
    hout = open('batch/test2.txt', 'w')
    hout.write(p.read())
    hout.close()
    p.close()
    p = os.popen('../../bake/cmdline.py -m -l -f bp/many_sprinkles.bp -o "@color@;purple"')
    ...
    hout.close()
    p.close()
    p = os.popen('../../bake/cmdline.py -m -l -f bp/nuts.bp -e "cat cookie.txt"')
